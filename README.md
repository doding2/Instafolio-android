...
광고 업계 포트폴리오를 인스타그램 이미지 형식으로 손쉽게 변환해주는 Android 앱
...
이미지 편집을 중심으로 한 앱 특성상  
대용량 리소스를 안정적으로 관리하는 것이 중요하다고 판단했습니다.

이에 따라 MVVM 아키텍처를 적용하여  
UI 로직과 비즈니스 로직을 분리하고,  
ViewModel을 중심으로 UI 상태를 관리하도록 설계했습니다.

또한 Room Database를 활용해  
최근 편집 이력을 로컬에 저장함으로써  
앱 재실행 이후에도 작업을 이어갈 수 있도록 구성했습니다.
